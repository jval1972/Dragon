  ...
  begin
    ...
    begin
      ...
      for iy := markp.y1 to markp.y2 do
        ...
        begin
          rtlp := @rtllightmap[iz * (LIGHTMAPSIZEX * LIGHTMAPSIZEY) + iy * LIGHTMAPSIZEX + markp.x1];
...
{        if l.shadow then
        begin
          iy1 := opengl2lightmapy(pdls.y - 2 * checkradious);
          if iy1 < 0 then
            iy1 := 0
          else if iy1 >= LIGHTMAPSIZEY then
            iy1 := LIGHTMAPSIZEY - 1;

          iy2 := opengl2lightmapy(pdls.y);
          if iy2 < 0 then
            iy2 := 0
        ...
        begin
          lo_ix := Abs(sqr(pdls.x - lightmap2openglx(ix)));
          for iy := iy1 to iy2 do
          begin
            lo_ixy := lo_ix + Abs(sqr(pdls.y - lightmap2opengly(iy)));
            ...
            begin
              ...
              begin
                ...
                squaredist := lo_ixy + Abs(sqr(pdls.z - lightmap2openglz(iz)));
                if squaredist <= squarecheck then
                begin
                  rtlp.shadow := l.shadow; // jval: wolf
                  if not rtlp.shadow then
                  begin
                    rtlp.r := rtlp.r + l.r;
                    rtlp.g := rtlp.g + l.g;
                    rtlp.b := rtlp.b + l.b;
    ...
    for iy := markp.y1 to markp.y2 do
      ...
      begin
        ...
        begin
          ...
          begin
            // Don't bother setting one more byte....
            PWord(point)^ := $8080;
            inc(point, 2);
            point^ := 128;
            inc(point);
