...
begin
  ...
  strafe := gamekeydown[key_strafe]{jval: wolf or
            (usemouse and mousebuttons[mousebstrafe]) or
            (usejoystick and joybuttons[joybstrafe])};
  speed := 0; // jval: WOLFintval(gamekeydown[key_speed] or joybuttons[joybspeed]);
  if autorunmode then
    speed := 1 - speed;
...
begin
  ...
  p.weaponowned[Ord(wp_plasma)] := 1;
  //p.weaponowned[Ord(wp_pistol)] := 1; // JVAL: wolf
  p.ammo[Ord(am_clip)] := 0; //jval: WOLF
