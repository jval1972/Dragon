...
procedure A_PlayerWalk(p: Pplayer_t);
...
uses
  ...
  info_h, info,
  m_rnd, m_vectors,
  p_enemy, p_mobj, p_inter, p_map, p_maputl, p_local, p_pspr, p_sounds, p_tick,
  r_main,
  sounds,
  s_sound,
  tables;
...
{  if P_AproxDistance(p.mo.x - p.lastsoundstepx,
  ...
  if walksnd < 0 then
    walksnd := S_GetSoundNumForName('player/walk');

  if walksnd > 0 then
    S_StartSound(nil, walksnd);
end;
